...
class Match(private val toMatch: List<Matchable>) : Statement() {
    ...
    companion object {
        fun <a, A : RedisNode<a>> QueryScope.match(node: A) = node.also { commands.add(Match(listOf(it))) }
        fun <a, b, A : RedisNode<a>, B : RedisNode<b>> QueryScope.match(node1: A, node2: B): Pair<A, B> {
